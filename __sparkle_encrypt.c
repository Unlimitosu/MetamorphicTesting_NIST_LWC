...
void __sparkle_AEAD_ProcessPlainText(SparkleState *state, uint8_t *out, const uint8_t *in, \
                      size_t inlen)
{
...
void __sparkle_AEAD_ProcessCipherText(SparkleState *state, uint8_t *out, const uint8_t *in, \
                       size_t inlen)
{
  ...
  const UChar *k)
{
  ...
  __sparkle_AEAD_Initialize(&state, k, npub);
  if (adsize) __sparkle_AEAD_ProcessAssocData(&state, ad, adsize);
  if (msize) __sparkle_AEAD_ProcessPlainText(&state, c, m, msize);
  __sparkle_AEAD_Finalize(&state, k);
  __sparkle_AEAD_GenerateTag(&state, (c + msize));
  *clen = msize;
  *clen += TAG_BYTES;
  ...
  const UChar *k)
{
  ...
  __sparkle_AEAD_Initialize(&state, k, npub);
  if (adsize) __sparkle_AEAD_ProcessAssocData(&state, ad, adsize);
  if (csize) __sparkle_AEAD_ProcessCipherText(&state, m, c, csize);
  __sparkle_AEAD_Finalize(&state, k);
  retval = __sparkle_AEAD_VerifyTag(&state, (c + csize));
  *mlen = csize;
