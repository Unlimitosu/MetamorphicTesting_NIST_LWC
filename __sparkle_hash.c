...
void __sparkle_ProcessMessage(SparkleState *state, const UChar *in, size_t inlen)
{
  ...
  while (inlen > RATE_BYTES) {  //16
    // addition of a message block to the state
      __sparkle_add_msg_blk(state, in, RATE_BYTES);
  ...
  __sparkle_add_msg_blk(state, in, inlen);
...
void __sparkle_add_msg_blk_PassUT(SparkleStateUT* state, const uint8_t* in, size_t inlen)
{
    uint32_t buffer[STATE_WORDS / 2] = { 0 };
    uint32_t tmpx = 0, tmpy = 0;
    int i;
...
void __sparkle_Initialize_PassUT(SparkleStateUT* state)
{
    memset(state, 0x00, sizeof(SparkleStateUT));
}

void __sparkle_ProcessMessage_PassUT(SparkleStateUT* state, 
                                     const UChar* in, size_t inlen) {
    // Set remain buffer
    uint32_t remain_buffer = state->current_index;

    while ((inlen + remain_buffer) > RATE_BYTES) {  
        memcpy(state->tmp + remain_buffer, in, RATE_BYTES - remain_buffer);
        __sparkle_add_msg_blk(state, state->tmp, RATE_BYTES);
        sparkle_ref(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
        inlen -= RATE_BYTES - remain_buffer;
        in += RATE_BYTES - remain_buffer;
        remain_buffer = 0;
    }
    memcpy(state->tmp + remain_buffer, in, inlen);
    remain_buffer += inlen;
    state->current_index = remain_buffer;
}

void __sparkle_Process_Lastblock_PassUT(SparkleStateUT* state) {
    // Hashing of Last Block
    state->y[(STATE_BRANS / 2) - 1] ^= ((state->current_index < RATE_BYTES) ? CONST_M1 : CONST_M2);
    __sparkle_add_msg_blk(state, state->tmp, state->current_index);
    sparkle_ref(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}

void __sparkle_Finalize_PassUT(SparkleStateUT* state, UChar* out)
{
    uint32_t buffer[DIGEST_WORDS];
    int i, outlen = 0;

    if (state->current_index != 0) {
        __sparkle_Process_Lastblock_PassUT(state);
    }
...
int __sparkle_crypto_hash(UChar *out, const UChar *in, ULLInt inlen)
{
  ...
  __sparkle_ProcessMessage(&state, in, insize);
  __sparkle_Finalize(&state, out);
...
{
    SparkleStateUT state;
    size_t insize = (size_t)inlen;

    __sparkle_Initialize_PassUT(&state);
    __sparkle_ProcessMessage_PassUT(&state, in, insize);
    __sparkle_Finalize_PassUT(&state, in, out);
