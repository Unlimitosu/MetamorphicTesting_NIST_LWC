#include "__isap_api.h"
//#include "__isap_crypto_hash.h"
#include "__isap_Ascon-reference.h"
...
int __isap_crypto_hash(unsigned char* out, const unsigned char* in,
...
// state size: ASCON_128_STATE_SZ
void __isap_Initialization(unsigned char* state) {
    Ascon_Initialize(state);
    Ascon_AddBytes(state, ASCON_HASH_IV, 0, ASCON_128_RATE);
    Ascon_Permute_Nrounds(state, ASCON_128_PA_ROUNDS);
}

void __isap_process(unsigned char* state, unsigned char* out, const unsigned char* in, unsigned long long len) {
    /* absorb full plaintext blocks */
    while (len >= ASCON_128_RATE) {
        Ascon_AddBytes(state, in, 0, ASCON_128_RATE);
        Ascon_Permute_Nrounds(state, ASCON_128_PA_ROUNDS);
...
void __isap_Finalize(unsigned char* state, unsigned char* out) {
    /* squeeze full output blocks */
